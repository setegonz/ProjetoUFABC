...
% Draw the image so that it is centered
   Screen('DrawTexture', window, images(ii), [], [centeredRect], 0);
    ...
    while true
        time = GetSecs; %time = toc
        RestrictKeysForKbCheck([spaceBar])
        clockPress = KbCheck;
        RestrictKeysForKbCheck([clockKey])
        [keyWasPressed, responseTime, ~] = KbCheck;
        
        if time - firstFlip < 60 && clockPress == true % display at� um minuto de experimento (apenas segundos)
        ...
        elseif time - firstFlip > 59 && clockPress == true %display de um minuto para frente (minutos e segundos)
            clockTime = time - firstFlip; % em segundos
            ...
            Screen('DrawTexture', window, images(ii), [],...
                ...
                (targetImageX + s2) (targetImageY + s1)], 0);
            Screen('Flip',[], shownClock + clockDuration);
        elseif time-stimulusStartTime > duration
